import {RunnableLambda, createStringProcessor} from '../lambda/runnable-lambda';
...
async function main() {
    ...
    console.log('测试用例 2：逐字符流式输出');
    const r = RunnableLambda.from((input: string) => input + ' (processed)');
    for await (const chunk of r.stream('hello')) {
        // 假设 invoke 返回的 Output 是字符串，则对字符串进行逐字符迭代
        for (const char of chunk.split('')) {
            console.log('  📄 逐字符输出:', char);
        }
    }
    console.log();
    ...
    try {
        console.log('使用超时配置 (1000ms)...');
        await slowProcessor.invoke('test', {timeout: 1000});
    } catch (error: any) {
